...
float HiddenWeights1[InputNodes+1][HiddenNodes1];
float OutputWeights[HiddenNodes+1][OutputNodes];
float OutputWeights1[HiddenNodes1+1][OutputNodes];
float HiddenDelta[HiddenNodes];
float HiddenDelta1[HiddenNodes1];
float OutputDelta[OutputNodes];
float OutputDelta1[OutputNodes];
...
Serial.println(port2);
 ...
 if(port1==1&&port2==1)
 {
 Serial.print("Pengenalan Gerbang XOR");
 Serial.println(" ");
 Serial.print("-----");
...
******************************************************************/
 ...
 if(port1==0&&port2==0)
 {
 Serial.print("Pengenalan JK FLIP FLOP");
 Serial.println(" ");
 Serial.print("-----");
  for( i = 0 ; i < HiddenNodes ; i++ ) {    
    for( j = 0 ; j <= InputNodes ; j++ ) { 
...
******************************************************************/
      ...
      for( i = 0 ; i < OutputNodes ; i++ ) {    
        ...
        Output[i] = 1.0/(1.0 + exp(-Accum)) ;   
        OutputDelta[i] = (Target1[p][i] - Output[i]) * Output[i] * (1.0 - Output[i]) ;   
        Error += 0.5 * (Target1[p][i] - Output[i]) * (Target1[p][i] - Output[i]) ;
      }
...
******************************************************************/
    ...
    for( i = 0 ; i < OutputNodes ; i++ ) {    
      ...
      Output[i] = 1.0/(1.0 + exp(-Accum)) ;  //Z, accum itu eror 
    }
    Serial.print ("  Output ");
    for( i = 0 ; i < OutputNodes ; i++ ) {       
   ...
   for( p = 0 ; p < PatternCount ; p++ ) { 
    ...
    for( i = 0 ; i < OutputNodes ; i++ ) {
     ...
     for( i = 0 ; i < HiddenNodes ; i++ ) {    
      Accum = HiddenWeights[InputNodes][i] ;
      for( j = 0 ; j < InputNodes ; j++ ) {
        Accum += masukan[p][j] * HiddenWeights[j][i] ;
      }
      Hidden[i] = 1.0/(1.0 + exp(-Accum)) ;
    }
